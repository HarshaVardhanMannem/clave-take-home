...
JOIN unified_locations ul ON uo.unified_location_id = ul.location_id
WHERE uo.voided = FALSE
GROUP BY ul.location_id, ul.location_code, ul.location_name, up.product_id, up.normalized_name, uc.category_name;
...
JOIN unified_locations ul ON uo.unified_location_id = ul.location_id
WHERE uo.voided = FALSE
GROUP BY ul.location_id, ul.location_code, ul.location_name;
...
-- Create regular views that point to materialized views for backward compatibility
-- This allows existing queries using v_daily_sales_summary to work without changes
-- Note: DROP first to avoid column name conflicts

DROP VIEW IF EXISTS v_daily_sales_summary CASCADE;
CREATE VIEW v_daily_sales_summary AS
SELECT * FROM mv_daily_sales_summary;

DROP VIEW IF EXISTS v_product_sales_summary CASCADE;
...
CREATE VIEW v_hourly_sales_pattern AS
SELECT 
    order_date,
    order_hour,
    day_of_week,
...
DROP VIEW IF EXISTS v_payment_methods_by_source CASCADE;
CREATE VIEW v_payment_methods_by_source AS
SELECT 
    source_system,
    payment_type,
    card_brand,
