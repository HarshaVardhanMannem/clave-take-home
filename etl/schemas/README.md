...
psql $DATABASE_URL -f etl/schemas/unified_schema.sql
```
...
1. Create the schema: `python etl/scripts/database/create_schema.py`
2. Create materialized views: `python etl/scripts/database/create_materialized_views.py` (required for analytics)
3. Ingest data: `python etl/scripts/pipeline/ingest_unified_data.py --all`
